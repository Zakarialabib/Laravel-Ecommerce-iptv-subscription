...
<div class="content-header">
    <div class="container mx-auto sm:px-4 max-w-full mx-auto sm:px-4">
        ...
        <div class="sm:w-1/2 pr-4 pl-4">
            <h1 class="flex flex-wrap pt-3 pb-3 py-4 px-4 mb-4 bg-gray-200 m-0 text-gray-900">{{ __('Offers') }} </h1>
        </div><!-- /.col -->
        <div class="sm:w-1/2 pr-4 pl-4">
                    ...
                    <div class="relative flex flex-col min-w-0 rounded break-words border bg-white border-1 border-gray-300 card-primary card-outline">
                            <div class="flex py-3 px-6 mb-0 bg-gray-200 border-b-1 border-gray-300 text-gray-900">
                                ...
                                <div class="flex w-1/2 justify-end">
                                    <a href="{{ route('admin.offer'). '?language=' . $currentLang->code }}" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            ...
                            <div class="flex-auto p-6">
                                <form class="form-horizontal" action="{{ route('admin.offer.update',  $offer->id) }}" method="POST">
                                    @csrf
                                        ...
                                        <div class="sm:w-4/5 pr-4 pl-4">
                                            <select class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm lang" name="language_id">
                                                @foreach($langs as $lang)
                                                    <option value="{{$lang->id}}" {{ $offer->language_id == $lang->id ? 'selected' : '' }} >{{$lang->name}}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="mb-4 flex flex-wrap ">
                                        ...
                                        <div class="sm:w-4/5 pr-4 pl-4">
                                            <textarea name="offer" class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-gray-800 border border-gray-200 rounded summernote" placeholder="{{ __('Offer') }}" rows="7">{{ $offer->offer }}</textarea>
                                            @if ($errors->has('offer'))
                                    ...
                                    <div class="mb-4 flex flex-wrap ">
                                        ...
                                        <div class="sm:w-4/5 pr-4 pl-4">
                                            <select class="block appearance-none w-full py-1 px-2 mb-1 text-base leading-normal bg-white text-gray-800 border border-gray-200 rounded" name="status">
                                               <option value="0" {{ $offer->status == '0' ? 'selected' : '' }}>{{ __('Unpublish') }}</option>
                                               <option value="1" {{ $offer->status == '1' ? 'selected' : '' }}>{{ __('Publish') }}</option>
                                              </select>
